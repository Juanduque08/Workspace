...
router.post('/', (req, res)=>{
    ...
    const query = `INSERT INTO estudiante(nombre, segundo_nombre, apellido,fecha_nacimiento, documento_identidad, genero)
                    VALUES (?,?,?,?,?,?)`;
    db.query(query, [nombre, segundo_nombre, apellido, fecha_nacimiento, documento_identidad, genero],
        (err, results) => {
            if (err){
                 console,error(err);
...
router.put('/:documento_identidad', (req, res) => {
    ...
    const query = `
        UPDATE estudiante
        SET nombre = ?, segundo_nombre = ?, apellido = ?, fecha_nacimiento = ?, documento_identidad = ?, genero = ?
        WHERE id = ?
    `;

    db.query(
        query,
        [nombre, segundo_nombre, apellido, fecha_nacimiento, documento_identidad, genero, id],
        (err, results) => {
            if (err) {
                console.error(err);
